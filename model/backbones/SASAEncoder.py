...
from modules import AttentionConv, AttentionStem
...
class Model(nn.Module):
    ...
    def forward(self, x):
        out = self.init(x)
        print(f"after init x's shape {out.shape}")
        out = self.layer1(out)
        print(f"after layer 1 x's shape {out.shape}")
        out = self.layer2(out)
        print(f"after layer 2 x's shape {out.shape}")
        out = self.layer3(out)
        print(f"after layer 3 x's shape {out.shape}")
        out = self.layer4(out)
        print(f"after layer 4 x's shape {out.shape}")
