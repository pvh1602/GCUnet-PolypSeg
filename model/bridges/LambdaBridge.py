...
from model.modules.modules import LambdaBlock
# helpers functions
...
class LambdaStack(nn.Module):
    def __init__(self, in_planes, planes, num_block=1, heads=4, dim_k=16, dim_u=1, r=7, n=None):
...
class LambdaBridge(nn.Module):
    def __init__(self, in_channels, out_channels, fmap_size=None, num_block=None, expansion=4):
        super(LambdaBridge, self).__init__()

        self.Lambda = LambdaStack(in_planes=in_channels, planes=int(out_channels/expansion), num_block=num_block, r=args.r)
...
if __name__ == '__main__':
    net = LambdaBridge(1024, 2048, None, 3).cuda()
    summary(net, (1024, 8, 8))